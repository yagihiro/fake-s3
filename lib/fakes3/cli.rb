...
module FakeS3
  class CLI < Thor
    ...
    method_option :reuse_address, :type => :boolean, :aliases => '-R', :desc => "Use SO_REUSEADDR option."
    ...
    def server
      ...
      server = FakeS3::Server.new(address,options[:port],store,hostname,ssl_cert_path,ssl_key_path,extra_options)
      server.serve
    end
